...
static bool rects_overlap (float left_a, float top_a, float right_a, float bottom_a,
                          float left_b, float top_b, float right_b, float bottom_b) {
    return left_a < right_b && right_a > left_b && top_a < bottom_b && bottom_a > top_b;
}

static void resolve_body_penetrations (Scene & scene, Vec2 & pos, comp::PhysicsBody & body, Vec2 & vel) {
    const float tile_size = (float)gwconst::SCREEN_BASE_TILESIZE_GAMEPIXELS;
    const float half_w = body.size.x * 0.5f;
    const float half_h = body.size.y * 0.5f;
    const float skin = std::max(body.skin, 0.0f);

    // Multiple tiny passes are more stable at tile corners than one large push,
    // especially where slope tips meet full solid tiles.
    for (int pass = 0; pass < 4; ++pass) {
        bool moved = false;

        const float left = pos.x - half_w;
        const float right = pos.x + half_w;
        const float top = pos.y - half_h;
        const float bottom = pos.y + half_h;

        const int tile_x_min = (int)std::floor((left - skin - 1.0f) / tile_size);
        const int tile_x_max = (int)std::floor((right + skin + 1.0f) / tile_size);
        const int tile_y_min = (int)std::floor((top - skin - 1.0f) / tile_size);
        const int tile_y_max = (int)std::floor((bottom + skin + 1.0f) / tile_size);

        for (const TileGrid & layer : scene.tile_layers) {
            for (int tx = tile_x_min; tx <= tile_x_max; ++tx) {
                for (int ty = tile_y_min; ty <= tile_y_max; ++ty) {
                    const CollisionType coll = layer.get_tile_coll(scene, tx, ty);
                    if (!is_solid_or_slope(coll, false)) continue;

                    const float tile_left = tx * tile_size;
                    const float tile_right = tile_left + tile_size;
                    const float tile_top = ty * tile_size;
                    const float tile_bottom = tile_top + tile_size;

                    if (!rects_overlap(pos.x - half_w, pos.y - half_h, pos.x + half_w, pos.y + half_h,
                    ...
                    const int slope_dir = get_tiletype_slope_dir(coll);
                    if (slope_dir != 0) {
                        // Sample both feet and the center so slope seams cannot drop the body
                        // into the tile when only one corner catches the surface.
                        const float samples[] = {
                            std::clamp(pos.x - half_w + skin + 1.0f, tile_left, tile_right),
                            std::clamp(pos.x, tile_left, tile_right),
                            std::clamp(pos.x + half_w - skin - 1.0f, tile_left, tile_right)
                        };

                        bool overlaps_surface = false;
                        float highest_surface = std::numeric_limits<float>::max();
                        for (float sample_x : samples) {
                            const float surface_y = slope_surface_y(tx, ty, sample_x, slope_dir);
                            if (bottom >= surface_y - skin && top < tile_bottom) {
                                highest_surface = std::min(highest_surface, surface_y);
                                overlaps_surface = true;
                            }
                        }

                        if (overlaps_surface) {
                            pos.y = highest_surface - half_h - skin;
                    ...
                    const float overlap_left = (pos.x + half_w) - tile_left;
                    const float overlap_right = tile_right - (pos.x - half_w);
                    const float overlap_top = (pos.y + half_h) - tile_top;
                    const float overlap_bottom = tile_bottom - (pos.y - half_h);

                    const float push_x = (overlap_left < overlap_right) ? -overlap_left : overlap_right;
                    const float push_y = (overlap_top < overlap_bottom) ? -overlap_top : overlap_bottom;

                    if (std::abs(push_y) <= std::abs(push_x)) {
                        pos.y += push_y + ((push_y < 0.0f) ? -skin : skin);
                        vel.y = 0.0f;
                        if (push_y < 0.0f) set_grounded(body, vel);
                    } else {
                        pos.x += push_x + ((push_x < 0.0f) ? -skin : skin);
                        vel.x = 0.0f;
                    }
                    moved = true;
                }
...
void PhysicsBodyMovementSystem::update (Registry & registry, float deltatime) {
    for (Entity entity : registry.view<comp::Transform, comp::Velocity, comp::PhysicsBody>()) {
        ...
        resolve_body_penetrations(scene, temp_pos, body, velocity);

        float settled_floor_y = 0.0f;
        if (find_body_floor(scene, temp_pos, body, body.skin + 1.0f, body.skin + 1.0f, settled_floor_y)) {
            temp_pos.y = settled_floor_y - (body.size.y * 0.5f) - body.skin;
            set_grounded(body, velocity);
        }
